...
$classpath = 'target/classes;' + (($guava, $gson, $jsoup, $sqlite, $slf4j, $logbackClassic, $logbackCore).FullName -join ';')
$out = 'target/server-out.log'
$err = 'target/server-err.log'

if (Test-Path $out) { Remove-Item $out }
if (Test-Path $err) { Remove-Item $err }

$proc = Start-Process -FilePath java -ArgumentList @('-cp', $classpath, 'org.CrawlUrlPhim.Main', '--server') -PassThru -RedirectStandardOutput $out -RedirectStandardError $err
try {
  Start-Sleep -Seconds 6

  $login = Invoke-RestMethod -Method Post -Uri 'http://localhost:8080/login' -ContentType 'application/json' -Body '{"username":"admin","password":"admin123"}'
  $prime = Invoke-RestMethod -Method Get -Uri 'http://localhost:8080/prime?n=10' -Headers @{ Authorization = ('Bearer ' + $login.token) }

  [pscustomobject]@{
    login_user = $login.username
    token_len = $login.token.Length
